...
public class AddQuizDtoAssemblerFactory extends AssemblerFactory<AddQuizDto, Quiz> {
  ...
  public HashMap<Class<?>, List> convert(AddQuizDto addQuizDto) {
    ...
    List<QuizQuestion> quizQuestionList = addQuizDto.getQuizQuestions()
            .stream()
            .map(addQuizQuestionDto -> {
              QuizQuestion quizQuestion = new QuizQuestion();
              quizQuestion.setQuiz(quiz);
              quizQuestion.setQuestion(addQuizQuestionDto.getQuestion());

              List<QuizAnswerQuestion> collect = getQuizAnswerQuestions(addQuizQuestionDto, quizQuestion);
              hashMap.put(QuizAnswerQuestion.class, collect);
              return quizQuestion;
            })
  ...
  private List<QuizAnswerQuestion> getQuizAnswerQuestions(AddQuizQuestionDto addQuizQuestionDto, QuizQuestion quizQuestion) {
    return addQuizQuestionDto.getQuizAnswerQuestions()
            .stream()
            .map(addQuizAnswerQuestionDto -> {
              ...
              quizAnswerQuestion.setStatus(addQuizAnswerQuestionDto.isCorrect() ?
                      AnswerStatus.CORRECT : AnswerStatus.NOT_CORRECT);
